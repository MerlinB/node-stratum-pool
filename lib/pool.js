...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    this.start = function(){
        SetupVarDiff();
        SetupApi();
        SetupBoostPowDaemonInterface(function(){
    ...
    function GetFirstJob(finishedCallback){
        ...
        GetBoostPowTemplate(function(error, result){
            if (error) {
            ...
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx){
                        ...
                        GetBoostPowTemplate(function(error, result, foundNewBlock){
                            if (foundNewBlock)
    ...
    function SetupBoostPowDaemonInterface(finishedCallback) {
        if (!options.boostPowDaemon){
            emitErrorLog('No poostPoW daemon has been configured - pool cannot start');
            return;
        }

        _this.boostPowDaemon = new daemon.interface([options.boostPowDaemon], function(severity, message){
            _this.emit('log', severity , message);
        });

        _this.boostPowDaemon.once('online', function(){
            finishedCallback();

        }).on('connectionFailed', function(error){
            emitErrorLog('Failed to connect boostPoW daemon: ' + JSON.stringify(error));
    ...
    /**
     * Fetches new block templates regularly to report to jobManager
     */
    ...
    /**
     * Requests new block template via RPC and sends it to jobManager to update current job
     */
    ...
    function getBoostPowTemplate(callback) {
        // TODO: Get boostPoW templates
    }
